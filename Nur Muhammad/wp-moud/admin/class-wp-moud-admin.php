...
class Wp_Moud_Admin {
    ...
    public function enqueue_styles() {
		 ...
		 * defined in Wp_Moud_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Wp_Moud_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

        wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/wp-moud-admin.css', array(), $this->version, 'all' );

        // CSS stylesheet for Color Picker
            wp_enqueue_style( 'wp-color-picker' );            
            wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/wp-moud-admin.css', array( 'wp-color-picker' ), $this->version, 'all' );
        
//        if ( 'settings_page_wp-moud' == get_current_screen() -> id ) {
//            
//        }
    }
    ...
    public function enqueue_scripts() {
        ...
        //        wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/wp-moud-admin.js', array( 'jquery' ), $this->version, false );

        wp_enqueue_media();   
            wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/wp-moud-admin.js', array( 'jquery', 'wp-color-picker' ), $this->version, false );   
...
//        }
    ...
    /**
    *
    * admin/class-wp-moud-admin.php - Don't add this
    *
    **/
    ...
    */
     ...
     *
     */
//        add_options_page( 'Moud\'s Announcement Plugin', 'WP Cleanup', 'manage_options', $this->plugin_name, array($this, 'display_plugin_setup_page')
//                        );
        
        
                add_menu_page( 'Moud\'s Announcement Plugin', 'Moud\'s Announcements', 'manage_options', $this->plugin_name, array($this, 'display_plugin_setup_page')
                        );
        
                
 add_submenu_page('wp-moud', 'Add New Announcement', 'Add New', 'manage_options', 'wp-moud/admin/partials/wp-moud-admin-display2.php');
 ...
 */
    ...
    public function display_plugin_setup_page() {
        include_once( 'partials/wp-moud-admin-display.php' );
    }

    public function display_plugin_setup_page2() {
        include_once( 'partials/wp-moud-admin-display2.php' );
    }
    ...
    public function __construct( $plugin_name, $version ) {
        ...
        $this->plugin_name = $plugin_name;
        $this->version = $version;
        $this->wp_moud_options = get_option($this->plugin_name);
     ...
     * @since    1.0.0
     */
    private function wp_moud_login_logo_css(){
        if(isset($this->wp_moud_options['login_logo_id']) && !empty($this->wp_moud_options['login_logo_id'])){
            $login_logo = wp_get_attachment_image_src($this->wp_moud_options['login_logo_id'], 'thumbnail');
    ...
    private function wp_moud_login_background_color(){
        if(isset($this->wp_moud_options['login_background_color']) && !empty($this->wp_moud_options['login_background_color']) ){
            $background_color_css  = "body.login{ background:".$this->wp_moud_options['login_background_color']."!important;}";
            return $background_color_css;
        }
    }
    // Get Button and links color is set and different from #00A0D2 return it's css
    private function wp_moud_login_button_color(){
        if(isset($this->wp_moud_options['login_button_primary_color']) && !empty($this->wp_moud_options['login_button_primary_color']) ){
            $button_color = $this->wp_moud_options['login_button_primary_color'];
    ...
    public function wp_moud_login_css(){
        if( !empty($this->wp_moud_options['login_logo_id']) || $this->wp_moud_login_background_color() != null || $this->wp_moud_login_button_color() != null){
            echo '<style>';
            if( !empty($this->wp_moud_options['login_logo_id'])){
                echo $this->wp_moud_login_logo_css();
            }
            if($this->wp_moud_login_background_color() != null){
                echo $this->wp_moud_login_background_color();
            }
            if($this->wp_moud_login_button_color() != null){
                echo $this->wp_moud_login_button_color();
            }
            echo '</style>';
        }
    }
